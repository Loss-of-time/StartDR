...
from ...schema.drugrec import DrugRecMedicine, DrugRecRecord
...
def build_gnn_train_sample(case: DrugRecCase) -> GNNTrainSample:
        ...
        evidences.append(
            {
                ...
                "drugid": drugid,
                "text": _build_evidence_text(medicine),
    ...
    for medicine in _get_ordered_evidence_medicines(case):
    ...
    return {
        "case": case,
        "model_input": {
            "patient_text": _build_patient_query_text(patient),
            "entities": entities,
            "evidences": evidences,
            "ent_to_ev": _normalize_columns(
                torch.tensor(ent_to_ev_rows, dtype=torch.float32)
            ),
            "ev_to_ent": _normalize_columns(
...
def _get_ordered_evidence_medicines(
    case: DrugRecCase,
...
def _append_entities_by_items[T](
    evidence_index: int,
    items: list[T],
    node_type: GNNNodeType,
...
def _build_patient_query_text(patient: DrugRecRecord) -> str:
    """按 TraceDR 风格拼接患者查询文本。"""
    return " || ".join(
        [
            str(patient["age"]),
            ",".join(patient["group"]),
            ...
            ",".join(
                medicine["name"] for medicine in patient["on_medicine"]
            ),
...
def _build_evidence_text(medicine: DrugRecMedicine) -> str:
    """按 TraceDR 风格拼接证据文本。"""
    treat_text = ", ".join(
        treat["treat"]
...
def _normalize_columns(matrix: torch.Tensor) -> torch.Tensor:
    """对邻接矩阵按列归一化。"""
    if matrix.numel() == 0:
        return matrix
