...
def main() -> None:
    ...
    batch_sizes: list[int] = cast(list[int], args.batch_sizes)

    print(f"开始读取 train 数据: {args.train_input.resolve()}")
    train_cases: FourSDrugStringCases = load_4sdrug_string_cases(args.train_input)
    print(f"开始读取 dev 数据: {args.dev_input.resolve()}")
    dev_cases: FourSDrugStringCases = load_4sdrug_string_cases(args.dev_input)
    print(f"开始读取 test 数据: {args.test_input.resolve()}")
    test_cases: FourSDrugStringCases = load_4sdrug_string_cases(args.test_input)
    print(
        "读取完成: "
    ...
    print(f"开始构造 4SDrug 离线产物，batch_sizes={sorted(set(batch_sizes))}")
